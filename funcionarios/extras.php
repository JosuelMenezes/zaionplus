<?php
// funcionarios/extras.php - Sistema Premium de Extras Salariais (CORRIGIDO)
// Iniciar sessão
...
// Função para formatar mês em português (substitui strftime)
...
try {
    ...
    // Estatísticas do mês
    $sql_stats = "SELECT 
                    COUNT(DISTINCT funcionario_id) as funcionarios_premiados,
                    COUNT(*) as total_extras_concedidos,
                    COALESCE(SUM(valor), 0) as valor_total_mes,
                    COALESCE(AVG(valor), 0) as valor_medio_extra
    ...
    <div class="row align-items-center">
        ...
        <div class="col-md-4">
            <select class="form-select form-select-lg" id="mesSelector" onchange="alterarMes()">
                <?php
                // Gerar opções dos últimos 12 meses (CORRIGIDO - sem strftime)
                for ($i = 0; $i < 12; $i++) {
